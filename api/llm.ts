...
async function callOpenRouter(prompt: string): Promise<string> {
  ...
  if (!message) throw new Error("Reponse vide d'OpenRouter");
  return message;
}
...
async function callMistral(prompt: string): Promise<string> {
  ...
  const data = await res.json();
  const message = data?.choices?.[0]?.message?.content;
  if (!message) throw new Error("Reponse vide de Mistral");
  return message;
}
...
async function callCohere(prompt: string): Promise<string> {
  ...
  if (!message) throw new Error("Reponse vide de Cohere");
  return message;
}
