...
app.post("/api/chats", async (req, res) => {
  ...
  writeDB(db);
  // CRITICAL: await Supabase sync so the chat exists on all Vercel instances
  // before the client sends the first message (prevents 404 errorCode 502)
  await supabaseWriteDB(db).catch(() => {});
  res.json(newChat);
});
...
app.post("/api/chats/:id/messages", async (req, res) => {
  ...
  // CRITICAL: Don't overwrite _db if it already contains the target chat (race condition:
  // chat created on instance A, Supabase write still in-flight, message sent to instance B
  // which syncs from Supabase BEFORE the write landed → chat not found → 404 errorCode 502)
  if (SUPABASE_URL && SUPABASE_KEY) {
    try {
      const restored = await Promise.race([
        supabaseReadDB(),
      ...
      if (restored && restored.chats) {
        const restoredHasChat = restored.chats.some(c => c.id === chatId);
        const localHasChat = _db?.chats?.some(c => c.id === chatId) ?? false;
        // Only overwrite if the restored version has the chat AND local doesn't
        // OR if local doesn't have the chat at all (fresh cold start)
        if (restoredHasChat || !localHasChat) {
          _db = restored;
          try { fs.writeFileSync(DB_PATH, JSON.stringify(restored, null, 2), "utf-8"); } catch {}
        }
      }
  ...
  let chatIndex = db.chats.findIndex(c => c.id === chatId);
  if (chatIndex === -1) {
    // Last resort: try reading from Supabase one more time (write may have landed late)
    if (SUPABASE_URL && SUPABASE_KEY) {
      try {
        const retryRestored = await supabaseReadDB();
        if (retryRestored?.chats?.some(c => c.id === chatId)) {
          _db = retryRestored;
          try { fs.writeFileSync(DB_PATH, JSON.stringify(retryRestored, null, 2), "utf-8"); } catch {}
          db = readDB();
          chatIndex = db.chats.findIndex(c => c.id === chatId);
        }
      } catch {}
    }
    if (chatIndex === -1) {
  ...
  const ENV_KEY_MAP: { env: string; provider: string; models: string[] }[] = [
    ...
    // OpenRouter RETIRÉ — clé sk-or-v1... invalide (401 "User not found"). Remettre quand Emerick aura une nouvelle clé.
    // { env: "OPENROUTER_API_KEY", provider: "openrouter", models: [
    //   "google/gemini-2.5-flash", "meta-llama/llama-3.3-70b-instruct", "qwen/qwen-2.5-72b-instruct",
